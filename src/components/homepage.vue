  ...
  <div>
    <el-tabs v-model="activeName" @tab-click="handleClick">
      <el-tab-pane label="  文章  " name="articles">
        <el-row >
    <el-col :span="18" >
      <el-card shadow="hover" class="park"  v-for="(item,index) in list" :key="item.id" :body-style="{ padding: '10px' }">
        <el-row>
          ...
          <div class="article-info">浏览数 ：{{item.viewCount}}        点赞数：{{item.likeCount}}        评论数：{{item.commentCount}}          创建时间：{{item.createTime}} </div>
          </el-col>
          <el-col :span="24" v-else>
            ...
            <div><el-link type="success" :href="'http://www.hhhbin.com/#/article/'+item.id" target="_blank">{{item.content}}...</el-link></div>
            <div class="article-info">浏览数 ：{{item.viewCount}}        点赞数：{{item.likeCount}}        评论数：{{item.commentCount}}          创建时间：{{item.createTime}} <el-tag type="danger" effect="plain" size="mini" v-if="index == 0">置顶</el-tag></div>
          </el-col>
          <el-col :span="8" v-if="item.firstImg != null">
              <el-image
              fit="cover"
              class="home-img"
                :src="item.firstImg"></el-image>
          </el-col>
    ...
    <el-col style="padding-left : 5px" class="park" :span="6">
      ...
      <el-link type="info" href="http://www.beian.miit.gov.cn/" target="_blank">青ICP备20000478号-1</el-link>
    </el-col>
  </el-row>
      </el-tab-pane>
      <el-tab-pane label="  提问  " name="ask">
        <el-row >
          <el-col :span="18">
        <el-card shadow="hover" class="park question-row"  v-for="item in asks" :key="item.id" :body-style="{ padding: '10px' }" >
          <router-link :to="'/'+item.user.name" v-if="item.user !== null">
            <el-avatar shape="square" :size="35" :src="item.user.imgUrl" class="park user-avatar"></el-avatar>
          </router-link>
          <el-avatar shape="square" :size="35" src="https://communityimg.cn-bj.ufileos.com/16f364c5-7e6d-4c0a-b554-c77da6b758fd.jpg?UCloudPublicKey=TOKEN_28669bbe-ba15-425a-a538-a2127d75f03d&Signature=2AgLBQppxzZPlLQk3vdnF%2BTALNg%3D&Expires=1619540019" class="park user-avatar" v-else></el-avatar>
          ...
          <span class="name" v-else>游客</span>
          <br/>
            <el-link type="success" :href="'http://www.hhhbin.com/#/question/'+item.id" target="_blank">
              <h1 class="title">
                  {{item.content}}
              </h1>
            </el-link>
    ...
    <el-col style="padding-left : 5px" class="park" :span="6">
      ...
      <el-link type="info" href="http://www.beian.miit.gov.cn/" target="_blank">青ICP备20000478号-1</el-link>
        ...
        <!-- <el-row >
    <el-col :span="18">
      <el-card shadow="hover" class="park"  v-for="item in asks" :key="item.id" :body-style="{ padding: '10px' }">
        <router-link :to="'/'+item.user.name" v-if="item.user !== null">
          <el-avatar shape="square" :size="35" :src="item.user.imgUrl" class="park user-avatar"></el-avatar>
        </router-link>
        <el-avatar shape="square" :size="35" src="https://communityimg.cn-bj.ufileos.com/16f364c5-7e6d-4c0a-b554-c77da6b758fd.jpg?UCloudPublicKey=TOKEN_28669bbe-ba15-425a-a538-a2127d75f03d&Signature=2AgLBQppxzZPlLQk3vdnF%2BTALNg%3D&Expires=1619540019" class="park user-avatar" v-else></el-avatar>
...
<script>
import { getScrollHeight, getScrollTop, getWindowHeight } from '../assets/js/screen.js'
export default {
  data () {
    return {
      pageInfo: {},
  ...
  methods: {
    async getasks () {
      ...
      if (res.code === 200) {
        console.log(this.askspageRequest.pageNum++)
    ...
    async submitAsk () {
      ...
      const { data: res } = await this.$http.post('ask', { content: this.textarea, token: this.token })
      console.log(res)
      if (res.code === 200) {
    ...
    load () {
      if (getScrollTop() + getWindowHeight() >= getScrollHeight()) {
        if (this.activeName === 'articles') {
          if (this.pageInfo.hasNextPage) { // 先判断下一页是否有数据
            this.getList()
          }
        } else {
          if (this.askPageInfo.hasNextPage) { // 先判断下一页是否有数据
            this.getasks()
          }
        }
      }
    ...
    async getList () {
      const { data: res } = await this.$http.post(this.activeName, this.pageRequest)
      if (res.code === 200) {
